...
Class unit
	...
	Method avoid()
		...
		For Local j:Int=0 Until myunit.Length
			If i<>j
			Local x1:Int=myunit.Get(i).px
			Local y1:Int=myunit.Get(i).py
			Local x2:Int=myunit.Get(j).px
			Local y2:Int=myunit.Get(j).py			
			If distance(x1,y1,x2,y2) < 16 
				Local a:Float = getangle(x2,y2,x1,y1)+Rnd(-Pi/2,Pi/2)
				myunit.Get(i).px += Cos(a)
				myunit.Get(i).py += Sin(a)
			End If
			End If
		Next
		Next
...
Class MyWindow Extends Window
	...
	Method New()
		...
		myunit.Push(New unit(70,50))
		myunit.Push(New unit(80,50))
